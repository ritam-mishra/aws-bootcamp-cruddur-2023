...
## Data Modelling
Single Table Design is a data modelling technique in which all related data is stored in a single database table. We use DynamoDB for the Direct Messaging System in our Cruddur application. We can break down the data access into four patterns:

* Pattern A for displaying messages. List of messages that belong to a message group can be seen by the users
* Pattern B for showing message groups. Users can see a list of message groups so they can check the other persons they have been talking to.
* Pattern C for creating a new message in a new message group.
* Pattern D for creating a new message in an existing message group.
...
First I restructured the bash scripts folders bin in the backend-flask folder. The files are: `backend-flask/bin/db`;  DynamoDB `backend-flask/bin/db`; AWS RDS `backend-flask/bin/rds` and AWS Cognito `backend-flask/bin/cognito`. 
Then I added `boto3` to `backend-flask/requirements.txt`.
...
VALUES
  ('Ritam Mishra', 'ritammishra2002@gmail.com', 'ritammishra' ,'MOCK'),
...
VALUES
  (
    (SELECT uuid from public.users WHERE users.handle = 'ritammishra' LIMIT 1),
    'This was imported as seed data!',
    current_timestamp + interval '10 day'
  )
  ```
* Then, I created the `backend-flask/bin/cognito/list-users` script.
```py
#!/usr/bin/env python3
...
* After that, I created the `backend-flask/bin/db/update_cognito_user_ids` script.
```py
#!/usr/bin/env python3
...
Under `bin/ddb` folder I also added a file named `seed.sql` which has a conversation between me and 'bayko'.  
```
